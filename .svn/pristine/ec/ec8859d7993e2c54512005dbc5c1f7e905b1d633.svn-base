<template>
    <div ref="bodyEl" class="enter-y my-index">
            ...
            <div class="md:grid xl:grid-cols-4 md:grid-cols-2 md:gap-16px card-block">
                ...
                <div>
                    <ul class="w-full -ml-2">
                        <li>
                            <div>
                                <a-input-search placeholder="请输入要查询的处方内容" enter-button="搜索" size="large"/>
                            </div>
                        </li>
                    </ul>
                </div>
        ...
        <div class="lg:w-1/3">
            <Card title="集采药品监管覆盖情况" :loading="loading" class="h-300px" :bodyStyle="{ padding: '1rem' }">
        ...
        <div class="lg:w-1/3">
            <Card title="特殊药品监管覆盖情况" :loading="loading" class="h-300px" :bodyStyle="{ padding: '1rem' }">
                <my-echarts :style="{ height: 'calc(100% + 2rem)', margin: '-1rem' }" :options="{
                    tooltip: { trigger: 'axis', formatter: tooltipFormatter() },grid:{containLabel:true,top:30,bottom:60,left:30,right:30},legend:{data:['2023年','2022年','2021年'],bottom:20},
                    xAxis: { type: 'category', data: ['品种总数', '已检查品种', '已抽样品种', '已追溯品种'] }, yAxis: { type: 'value' },
                    series: [{ type: 'bar', name: '2023年', barWidth: 15, data: [415,...randomArray(3,1,415)] }
                        ,{ type: 'bar', name: '2022年', barWidth: 15, data: [415,...randomArray(3,1,415)] }
                        ,{ type: 'bar', name: '2021年', barWidth: 15, data: [415,...randomArray(3,1,415)] }
                    ]}"/>
            </Card>
        </div>
        <div class="lg:w-1/3">
            <Card title="生物制品监管覆盖情况" :loading="loading" class="h-300px" :bodyStyle="{ padding: '1rem' }">
                <my-echarts :style="{ height: 'calc(100% + 2rem)', margin: '-1rem' }" :options="{
                    tooltip: { trigger: 'axis', formatter: tooltipFormatter() },grid:{containLabel:true,top:30,bottom:60,left:30,right:30},legend:{data:['2023年','2022年','2021年'],bottom:20},
                    xAxis: { type: 'category', data: ['品种总数', '已检查品种', '已抽样品种', '已追溯品种'] }, yAxis: { type: 'value' },
                    series: [{ type: 'bar', name: '2023年', barWidth: 15, data: [99,...randomArray(3)] }
                        ,{ type: 'bar', name: '2022年', barWidth: 15, data: [99,...randomArray(3)] }
                        ,{ type: 'bar', name: '2021年', barWidth: 15, data: [99,...randomArray(3)] }
                    ]}"/>
            </Card>
        </div>
        <div class="lg:w-1/3">
            <Card title="药品生产许可事项统计" :loading="loading" class="h-300px" :bodyStyle="{ padding: '1rem' }">
                <my-echarts :style="{ height: 'calc(100% + 2rem)', margin: '-1rem' }" :options="{
                    ...
                    series: [{ type: 'bar', stack: '核发', name: '核发(A类)', barWidth: 10, data: randomArray(6) }
                        ,{ type: 'bar', stack: '核发', name: '核发(B类)', data: randomArray(6) }
                        ,{ type: 'bar', stack: '核发', name: '核发(C类)', data: randomArray(6) }
                        ,{ type: 'bar', stack: '核发', name: '核发(D类)', data: randomArray(6) }
                        ,{ type: 'bar', stack: '变更', name: '基本信息变更', barWidth: 10, data: randomArray(6) }
                        ,{ type: 'bar', stack: '变更', name: '生产地址及名称变更', data: randomArray(6) }
                        ,{ type: 'bar', stack: '变更', name: '生产范围变更', data: randomArray(6) }
                        ,{ type: 'bar', stack: '变更', name: '生产地址及范围变更(委托)', data: randomArray(6) }
                        ,{ type: 'bar', stack: '变更', name: '生产地址及范围变更(受托)', data: randomArray(6) }
                        ,{ type: 'bar', stack: '延续', name: '换发', barWidth: 10, data: randomArray(6) }
                        ,{ type: 'bar', stack: '延续', name: '补发', data: randomArray(6) }
                        ,{ type: 'bar', stack: '延续', name: '注销',data: randomArray(6) }
                    ]}"/>
            </Card>
        </div>
        <div class="lg:w-1/3">
            <Card title="企业变更次数最多的前10家企业" :loading="loading" class="h-300px" :bodyStyle="{ padding: '1rem' }">
                <my-echarts :style="{ height: 'calc(100% + 2rem)', margin: '-1rem' }" :options="{
        ...
        <div class="lg:w-1/3">
            <Card title="品种变更次数最多的前10个品种" :loading="loading" class="h-300px" :bodyStyle="{ padding: '1rem' }">
                <my-echarts :style="{ height: 'calc(100% + 2rem)', margin: '-1rem' }" :options="{
                    tooltip: { trigger: 'axis', formatter: tooltipFormatter(['次']) },grid:{containLabel:true,top:20,bottom:20,left:30},
        ...
        <div class="lg:w-1/3">
            <Card title="监督检查完成情况" :loading="loading" class="h-300px" :bodyStyle="{ padding: '1rem' }">
                <my-echarts :style="{ height: 'calc(100% + 2rem)', margin: '-1rem' }" :options="{
                    tooltip: { trigger: 'axis', formatter: tooltipFormatter() },grid:{containLabel:true,top:30,bottom:30,left:30,right:30},
                    xAxis: { type: 'category', axisLabel:{rotate:45}, data: ['武汉分局','黄石分局','十堰分局','宜昌分局','襄阳分局','鄂州分局','荆门分局','孝感分局','荆州分局','黄冈分局','咸宁分局','随州分局','恩施分局','汉江分局'] }, yAxis: { type: 'value' },
                    series: [
                        { type: 'bar', name: '已完成检查任务的企业数', data: randomArray(14) }
                        ,{ type: 'bar', name: '未完成检查任务的企业数', data: randomArray(14) }
                        /*{ type: 'bar', stack: '常规检查', name: '常规检查-已检企业', data: randomArray(14) }
                        ,{ type: 'bar', stack: '常规检查', name: '常规检查-未检企业', data: randomArray(14) }
                        ,{ type: 'bar', stack: 'GMP符合性检查', name: 'GMP符合性检查-已检企业', data: randomArray(14) }
                        ,{ type: 'bar', stack: 'GMP符合性检查', name: 'GMP符合性检查-未检企业', data: randomArray(14) }
                        ,{ type: 'bar', stack: '专项检查', name: '专项检查-已检企业', data: randomArray(14) }
                        ,{ type: 'bar', stack: '专项检查', name: '专项检查-未检企业', data: randomArray(14) }
                        ,{ type: 'bar', stack: '药物警戒检查', name: '药物警戒检查-已检企业', data: randomArray(14) }
                        ,{ type: 'bar', stack: '药物警戒检查', name: '药物警戒检查-未检企业', data: randomArray(14) }*/
                    ]}"/>
            </Card>
        </div>
        <div class="lg:w-1/3">
        ...
        <div class="lg:w-1/3">
            <Card title="稽查执法违法行为统计" :loading="loading" class="h-300px" :bodyStyle="{ padding: '1rem' }">
                <my-echarts :style="{ height: 'calc(100% + 2rem)', margin: '-1rem' }" :options="{
                    tooltip: { trigger: 'axis', formatter: tooltipFormatter(['次']) },grid:{containLabel:true,top:20,bottom:20,left:30},
                    xAxis: { type: 'value' }, yAxis: [{ type: 'category', data: ['生产或销售假药、劣药','未遵守药品生产质量管理规范','应当办理变更登记手续而未办理','违反药品召回管理规定','未取得《药品生产许可证》生产药品'] }],
                    series: [
                        { type: 'bar', name: '发生次数', data: [9,6,6,5,5] }
                    ]}"/>
            </Card>
        </div>
