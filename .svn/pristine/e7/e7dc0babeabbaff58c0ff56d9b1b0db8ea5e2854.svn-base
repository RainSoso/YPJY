  ...
  routes.forEach((item) => {
    ...
    if (component) {
      ...
      if (layoutFound) {
        ...
        /* ----------解决同一个组件多路由使用时无法缓存的问题------------ */
        item.component = dynamicRouteComponent(dynamicImport(dynamicViewsModules, component as string), item.name);
      }
    } else if (name) {
...
export function dynamicRouteComponent(componentOptions, routeName) {
  const clone = (comp) => {
    if (comp && comp.default) {
      if (routeName != comp.default.name) {
        return { ...comp.default, name: routeName };
      }
    } else if (comp instanceof Function) {
      return () => {
        return dynamicRouteComponent(comp(), routeName);
      };
    }
    return comp;
  };
