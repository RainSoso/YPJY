<template>
    <div class="bg-white m-4 p-4">
        <h2 class="text-center text-2xl font-600 p-4 !mb-0">辖区集采品种监管情况</h2>
...
function loadData(params = { year: currentYear.value, isJc: 'jcpz' }) {
...
function toPzListPage(item, tagTitle = '', query = {}) {
    go({ path: '/ypyd/jcpz/j', query: { scAreaCode: item.xzqhdm, year: currentYear.value, tagTitle: getAreaName(item.xzqhdm) + (tagTitle ? ' - ' + tagTitle : '') + ' - 集采品种', ...query } });
}
