...
function loadData(params = { year: currentYear.value, isJc: 'qt' }) {
...
function toPzListPage(item, tagTitle = '', query = {}) {
    go({ path: '/zdjg/qtyp/pzxx', query: { scAreaCode: item.xzqhdm, year: currentYear.value, tagTitle: getAreaName(item.xzqhdm) + (tagTitle ? ' - ' + tagTitle : '') + ' - 其他药品', ...query } });
}
