 ...
 * 根据code获取下拉option的数据
 * @param code
 * @param ignore01 是否忽略市辖区
 * @returns []
 */
function getDataByCode(code, ignore01 = false) {
  let data = [];
  for (const prop in REGION_DATA[code]) {
    if (ignore01 && prop.endsWith('01') && REGION_DATA[code][prop] == '市辖区') continue;
    data.push({
      value: prop, // 省份code值
 ...
 * @returns {Array}
 */
function getRealCode(code, level = null) {
  let arr = [];
...
function getPcode(id, arr, index) {
  for (let item of pca) {
    if (item.id === id && (null == index || item.index == index)) {
      arr.unshift(id);
      if (item.pid != '86') {
        getPcode(item.pid, arr, null != index ? --index : null);
      }
    }
  }
}
