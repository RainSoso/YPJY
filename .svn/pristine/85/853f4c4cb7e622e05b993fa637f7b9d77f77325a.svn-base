...
export function adjustFormValues(props, values) {
  for (let key in values) {
    if (values[key] instanceof Array) {
      let schema = props?.schemas?.filter((item) => item.field === key)[0];
      if (schema?.component =='RangePicker') {
          values[key] = values[key].map((item) => {
              return dateUtil(item).format('yyyy-MM-DD HH:mm:ss.ms');
          });
          if (schema?.rangeFields) {
            values[schema?.rangeFields[0]] = values[key][0];
            values[schema?.rangeFields[1]] = values[key][1];
          }
      }
