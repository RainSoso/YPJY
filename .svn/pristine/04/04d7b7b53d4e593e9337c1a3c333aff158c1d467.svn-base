...
<script lang="ts" setup>
  ...
  import { useBasicFormSchema, getOrgCategoryOptions } from '../depart.data';
  ...
  const [registerModal, { setModalProps, closeModal }] = useModalInner(async (data) => {
    ...
    // 当前是否为添加子级
    let isChild = unref(data?.isChild), orgCategory = unref(data?.record?.orgCategory);
    let categoryOptions = getOrgCategoryOptions(isChild && orgCategory);
    // 隐藏不需要展示的字段
    updateSchema([
      {
        field: 'parentId',
