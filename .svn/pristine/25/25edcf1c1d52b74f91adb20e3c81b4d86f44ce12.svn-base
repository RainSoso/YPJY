...
import { list, getImportUrl, getExportUrl } from '../TJcQyTjxx.api';
...
function getGo(record: Recordable) {
  if (record.zslx == '6' || record.zslx == '201010') {
    go({ path: `/yqyd/ypsc/TJcQyJbxxList/${encodeURIComponent(record.qymc || '')}` + '/' + record.shxydm, query: { forceTags: unref(queryForceTags), isCyr: 'true', year: record.year } });
  } else {
    go({ path: `/yqyd/ypsc/TJcQyJbxxList/${encodeURIComponent(record.qymc || '')}` + '/' + record.shxydm, query: { forceTags: unref(queryForceTags), year: record.year } });
  }
}
