<template>
  <div class="homeBody">
    <a-row :gutter="10">
      <a-col :span="17">
        <div class="gutter-head flex flex-col">
          ...
          <div class="h-16/24 flex flex-row px-2">
            <div class="w-16/24 mr-1 flex flex-col   ">
            ...
            <div class="w-8/24 ml-1 flex flex-col">
              <div class="h-1/24  flex flex-row  items-center my-2">
                <div class="jcqk-title">检查情况({{ xzqhdmName }})</div>
              </div>

              <div class="h-8/24  mt-2 flex flex-row">
              ...
              <div class="h-6/24 jcqk-border flex flex-row justify-around">
          ...
          <div class="h-2/24  flex flex-row justify-between  items-center px-2">
            <div class="jcqk-title">上年度检查情况</div>
            <div>
          ...
          <div class="h-4/24 flex flex-row justify-around px-2">
            <div class="w-1/6 mr-1 p-4 flex flex-row relative" style="background-color: #EEFBEE;">
              <div class="w-full flex flex-col justify-center cursor-pointer" @click="goxbwjc()">
                <div class="jcqk-title-sl">上一年</div>
      ...
      <a-col :span="7" v-show="true">
        <a-row>
          <a-col class="gutter-right-top" :span="24">
            <div class="flex flex-col p-3">
              <div class="flex flex-row justify-between items-center">
                <div class="qylx-title">
                  <SvgIcon size="18" name="ypjy-qylx-title" />信息咨询
                </div>
                <div>
                  <a-radio-group v-model:value="xxzx_lb" @change="homeXxzx()" buttonStyle="solid" size="small">
                    <a-radio-button value="1">通知公告</a-radio-button>
                    <a-radio-button value="2">法律法规</a-radio-button>
                  </a-radio-group>
                </div>
              </div>
              <div class="my-2">
                <a-list :split="false" :data-source="tzggData">
                  <template #renderItem="{ item, index }">
                    <a-list-item :style="{ backgroundColor: index % 2 == 0 ? '#EDF3FF' : '#ffffff' }">
                      <div class="flex flex-row w-full pr-2 justify-between cursor-pointer" @click="gotzgg()">
                        <div class="list-content" :title="item.title">{{ item.title }}</div>
                        <div class="list-content-date">{{ formatToDate1(item.fbrq) }}</div>
                      </div>
                    </a-list-item>
                  </template>
                </a-list>
          ...
          <a-col class="gutter-right-bottom" :span="24">
            <div class="flex flex-col p-3">
              <div class="  flex flex-row justify-between items-center">
                <div class="qylx-title">
                  <SvgIcon size="18" name="ypjy-qylx-title" />不合格检查项目排行
                </div>
                <div>
                  <a-radio-group v-model:value="bhgxm_lb" @change="homeBhgxm()" buttonStyle="solid" size="small">
                    <a-radio-button value="11">药品批发</a-radio-button>
                    <a-radio-button value="18">连锁总部</a-radio-button>
                  </a-radio-group>
                </div>
              ...
              <div class="my-2 ">
                <a-list :split="false" :data-source="bhgxmData">
                  <template #renderItem="{ item, index }">
                    <a-list-item :style="{ backgroundColor: index % 2 == 0 ? '#EDF3FF' : '#ffffff' }">
                      <div class="flex flex-row w-44/48 pr-2  pl-2">
                        <div class="list-content-index flex  items-center">
                          <SvgIcon size="24" name="ypjy-list-xh" />
                          <span class="absolute list-content-index"> {{ index + 1 }}</span>
                        </div>
                        <div class="list-content" :title="item.name">{{ item.name }}</div>
                      </div>
                      <div class="list-content-sl">{{ item.value }}</div>
                    </a-list-item>
                  </template>
                </a-list>
              </div>
...
const mapOption = reactive({
  ...
  visualMap: {
    ...
    map: 'HB', aspectScale: 1, zoom: 0.8, animationDurationUpdate: 0, layoutCenter: ['50%', '49%'], layoutSize: '130%', roam: false, silent: true, emphasis: { label: { show: false } }
    , itemStyle: { borderWidth: 0, borderColor: 'transparent', shadowColor: '#8cd3ef', shadowOffsetY: 10, shadowBlur: 120, areaColor: 'transparent' }
  }, {
    map: 'HB', zlevel: -1, aspectScale: 1, zoom: 0.8, animationDurationUpdate: 0, layoutCenter: ['50%', '50%'], layoutSize: '130%', roam: false, silent: true
    ...
    map: 'HB', zlevel: -2, aspectScale: 1, zoom: 0.8, animationDurationUpdate: 0, layoutCenter: ['50%', '51%'], layoutSize: '130%', roam: false, silent: true
    ...
    map: 'HB', zlevel: -3, aspectScale: 1, zoom: 0.8, animationDurationUpdate: 0, layoutCenter: ['50%', '52%'], layoutSize: '130%', roam: false, silent: true
    ...
    map: 'HB', zlevel: -4, aspectScale: 1, zoom: 0.8, animationDurationUpdate: 0, layoutCenter: ['50%', '53%'], layoutSize: '130%', roam: false, silent: true
    ...
    setOptionsMapChart(mapOption, true, (res) => {
      ...
      res.on('click', (res) => {
        ...
        Tools.throttle(() => {
          ...
          let activeItem = geo.features.find(item => item.properties.adcode == res.data.xzqhdm);
          if (activeItem && activeItem.properties.adcode && activeItem.properties.childrenNum) {
            //判断是否可以下钻
            let temp = JSON.parse(JSON.stringify(hbFullGeo));
            temp.features = temp.features.filter(item => item.properties.parent.adcode == res.data.xzqhdm);
            registerMap('HB', temp);
            loadHomehead(hbFullGeo, { areaCode: res.data.xzqhdm }, false, true);
          } else {
            loadHomehead(hbFullGeo, { areaCode: res.data.xzqhdm }, false, false);
          }
          homeSndqk({ areaCode: res.data.xzqhdm });
        });
      });
...
function homeSndqk(params = {}) {
  defHttp.get({ url: '/report/ypjyHomePage/homeSndqk', params: { year: currentYear.value, zslx: jcqk_lb.value, ...params } }).then((res) => {
    sndqkData.value = res;
  })
}
function homeFlfg() {
...
function goxbwjc() {
  if (sndqkData.value.xkbwjc != 0) {
    go({ path: `/ypjy/homePage/xbwjc/` + jcqk_lb.value + '/' + currentYear.value + '/' + xzqhdm.value + '/xbwjc' });
  }
}

function gowjcqy() {
  if (sndqkData.value.sndwjc != 0) {
    go({ path: `/ypjy/homePage/wjcqy/` + jcqk_lb.value + '/' + currentYear.value + '/' + xzqhdm.value + '/wjcqy' });
  }
}
...
function gojcbhg() {
  if (sndqkData.value.jcbhg != 0) {
    go({ path: `/ypjy/homePage/jcbhg/` + jcqk_lb.value + '/' + currentYear.value + '/' + xzqhdm.value + '/jcbhg' });
  }
}
//抽样不合格
function gocybhg() {
  if (sndqkData.value.cjbhg != 0) {
    go({ path: `/ypjy/homePage/cybhg/` + jcqk_lb.value + '/' + currentYear.value + '/' + xzqhdm.value + '/cybhg' });
  }
}
...
function gondzfcf() {
  if (sndqkData.value.ndzfcf != 0) {
    go({ path: `/ypjy/homePage/ndzfcf/` + jcqk_lb.value + '/' + currentYear.value + '/' + xzqhdm.value + '/ndzfcf' });
  }
}
