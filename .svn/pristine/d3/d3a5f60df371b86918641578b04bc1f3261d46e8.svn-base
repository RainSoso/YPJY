...
export function useSelectBiz(getList, props) {
  ...
  function getValue(item) {
    return props.rowKeyFun ? props.rowKeyFun(item, props.rowKey) : item[props.rowKey];
  }

  function getLabel(item){
    return isFunction(props.labelKey) ? props.labelKey(item) : item[props.labelKey];
  }
  ...
  async function getDataSource(params, flag) {
    if (!params[props.rowKey]) return;
    let { records } = await getList(params);
    dataSource.value = records;
    if (flag) {
      let options = <any[]>[];
      records.forEach((item) => {
        options.push({ label: getLabel(item), value: getValue(item), source: item });
      });
