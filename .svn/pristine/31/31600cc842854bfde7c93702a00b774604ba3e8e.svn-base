...
function fromUrlParams(url: string) {
  let res = {};
  url = (url || '').trim();
  if (!url || 0 > url.indexOf('?')) return null;
  url = url.substring(url.indexOf('?') + 1);
  url.split('&').forEach(item => {
    if (0 > item.indexOf('=')) {
      res[item] = '';
    } else {
      let val = item.substring(item.indexOf('=') + 1).replaceAll(/\+/ig, ' ');
      res[item.substring(0, item.indexOf('='))] = decodeURIComponent(val);
    }
  });
  return res;
}
...
const getToTarget = (tabItem: RouteLocationNormalized) => {
  const { params, path, fullPath } = tabItem;
  return {
    params: params || {},
    path,
    query: fromUrlParams(fullPath) || {},
  };
};
