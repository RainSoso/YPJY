<template>
    <div class="bg-white m-4 p-4">
        <h2 class="text-center text-2xl font-600 p-4">辖区中药药品监管情况</h2>
...
function loadData(params = { year: currentYear.value, isJc: 'zyzj' }) {
...
function toPzListPage(item, tagTitle = '', query = {}) {
    go({ path: '/zdjg/zyzj/zjzjxx', query: { scAreaCode: item.xzqhdm, year: currentYear.value, tagTitle: getAreaName(item.xzqhdm) + (tagTitle ? ' - ' + tagTitle : '') + ' - 中药药品', ...query } });
}
