<template>
    <div class="bg-white m-4 p-4">
        <h2 class="text-center text-2xl font-600 p-4 !mb-0">辖区特殊品种监管情况</h2>
            ...
            <tbody id="table-body">
                <tr v-for="(item, idx) in Data" :key="idx">
                    ...
                    <td class="text-center">
                        ...
                        / <a-button type="link" class="!px-1" @click="toPzListPage(item, '停产', {sczk: '0'})">{{ item.tcpzNum }}</a-button>
                        ）
                    </td>
                    <td class="text-center">
...
function loadData(params = { year: currentYear.value, isJc: 'tsyp' }) {
...
function toPzListPage(item, tagTitle = '', query = {}) {
    go({ path: '/zdjg/tsyp/tsypxx', query: { scAreaCode: item.xzqhdm, year: currentYear.value, tagTitle: getAreaName(item.xzqhdm) + (tagTitle ? ' - ' + tagTitle : '') + ' - 特殊药品', ...query } });
}

function onExportXls() {
    let link = document.createElement("a");
    link.href = 'data:application/vnd.ms-excel;charset=utf-8,\ufeff' + encodeURIComponent(function () {
        let header = ['年度', '辖区', '监督检查完成率（应检/已检/未检/停产品种）', '抽检验完成率（应抽/已抽/未抽）', '药品追溯率（应追溯/已追溯/未追溯）', '扫码率'],
            body: string[] = [];
