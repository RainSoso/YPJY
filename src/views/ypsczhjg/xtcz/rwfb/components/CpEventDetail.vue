<template>
  <PageWrapper>
    <template #headerContent>
      ...
      <table class="my-table">
        ...
        <tr v-if="cpEvent.category < 3">
          ...
          <td colspan="3">
            <a-input v-model:value="cpEvent.title" :disabled="cpEvent.state > 9"></a-input>
          </td>
        </tr>
        <tr v-if="cpEvent.category == 3">
          <td class="text-blue font-600 text-right">事项标题</td>
        ...
        <tr v-if="cpEvent.category < 3">
          ...
          <td>
            <a-DatePicker v-model:value="cpEvent.closingDate" :disabled="cpEvent.state > 9"></a-DatePicker>
          </td>
        </tr>
        <tr v-if="cpEvent.category < 3">
          <td class="text-blue font-600 text-right">内容</td>
          <td colspan="3">
            <a-input v-model:value="cpEvent.content" :disabled="cpEvent.state > 9"></a-input>
          </td>
        </tr>
        <tr v-if="cpEvent.category < 3">
          <td class="text-blue font-600 text-right">备注</td>
          <td colspan="3">
            <a-textarea v-model:value="cpEvent.remarks" :disabled="cpEvent.state > 9"></a-textarea>
          </td>
        </tr>
        <tr v-if="cpEvent.category < 3">
          ...
          <td>
            <j-upload v-model:value="cpEvent.attaches" text="上传" number="1" :bizPath="'xtcz'"
              :disabled="cpEvent.state > 9"></j-upload>
          </td>
        </tr>
      </table>
    </template>
    <template #footer>
        ...
        <div>
          <div class="panel-heading">
            <div class="panel" style="margin-top: 20px">
              反馈信息
              <a-button type="primary" style="float: right; margin-top: 5px"
                v-if="cpEvent.state <= 10 || !cpEvent.state" @click="addFkxx"> 添加反馈信息</a-button>
            </div>
          </div>
          <div class="my-table">
            <colgroup>
            ...
            <table style="width: 100%" class="my-table text-center">
              ...
              <tr v-for="item in eventItems" :key="item">
                ...
                <td>
                  <span><a @click="deleteOnefkxx(item)">删除</a></span>
                </td>
              </tr>
            </table>
          </div>
      ...
      <div v-if="cpEvent.category == '2'">
        <div class="panel-heading">
          <div class="panel" style="margin-top: 20px">国抽药品检验信息
            <a-button type="primary" @click="selectPz" style="float: right; margin-top: 5px" v-if="cpEvent.state < 10">
              品种选择</a-button>
          </div>
        </div>
        <table class="my-table">
          ...
          <tr>
            ...
            <td colspan="3">
              <a-input v-model:value="cpEvent.gcfxcz.jhmc" placeholder="请输入计划名称" :disabled="cpEvent.state > 9" />
            </td>
          </tr>
          <tr>
            <td class="text-blue font-600 text-right">品种名称</td>
            <td>
              <a-input v-model:value="cpEvent.gcfxcz.pzmc" placeholder="请输入品种名称" :disabled="cpEvent.state > 9" />
            </td>
            <td class="text-blue font-600 text-right">药品通用名称</td>
            <td>
              <a-input v-model:value="cpEvent.gcfxcz.yptymc" placeholder="请输入药品通用名称" :disabled="cpEvent.state > 9" />
            </td>
          </tr>
          <tr>
            <td class="text-blue font-600 text-right">标示生产企业名称</td>
            <td>
              <a-input v-model:value="cpEvent.gcfxcz.bsscqymc" placeholder="请输入标示生产企业名称"
                :disabled="cpEvent.state > 9" />
            </td>
            <td class="text-blue font-600 text-right">生产企业所在省份</td>
            <td>
              <a-input v-model:value="cpEvent.gcfxcz.scqyszsf" placeholder="请输入生产企业所在省份"
                :disabled="cpEvent.state > 9" />
            </td>
          </tr>
          <tr>
            ...
            <td>
              <a-input v-model:value="cpEvent.gcfxcz.sjypph" placeholder="请输入涉及的样品批号" :disabled="cpEvent.state > 9" />
            </td>
            <td class="text-blue font-600 text-right">涉及样品批数</td>
            <td>
              <a-input v-model:value="cpEvent.gcfxcz.sjypps" placeholder="请输入涉及样品批数" :disabled="cpEvent.state > 9" />
            </td>
          </tr>
          <tr>
            <td class="text-blue font-600 text-right">药品批准文号</td>
            <td>
              <a-input v-model:value="cpEvent.gcfxcz.yppzwh" placeholder="请输入药品批准文号" :disabled="cpEvent.state > 9" />
            </td>
            <td class="text-blue font-600 text-right">风险类别</td>
            <td>
              <a-input v-model:value="cpEvent.gcfxcz.fxllb" placeholder="请输入风险类别" :disabled="cpEvent.state > 9" />
            </td>
          </tr>
          <tr>
            <td class="text-blue font-600 text-right">研究结果描述及原因分析</td>
            <td colspan="3">
              <a-textarea v-model:value="cpEvent.gcfxcz.jgfx" placeholder="请输入研究结果描述及原因分析"
                :disabled="cpEvent.state > 9" />
            </td>
          </tr>
          <tr>
            ...
            <td>
              <a-input v-model:value="cpEvent.gcfxcz.jydw" placeholder="请输入检验单位" :disabled="cpEvent.state > 9" />
            </td>
            <td class="text-blue font-600 text-right">检验单位电话</td>
            <td>
              <a-input v-model:value="cpEvent.gcfxcz.jydwdh" placeholder="请输入检验单位电话" :disabled="cpEvent.state > 9" />
            </td>
          </tr>
          <tr>
            <td class="text-blue font-600 text-right">国家局处理情况</td>
            <td colspan="3">
              <a-input v-model:value="cpEvent.gcfxcz.gjjclqk" placeholder="请输入国家局处理情况" :disabled="cpEvent.state > 9" />
            </td>
          </tr>
          <tr>
            <td class="text-blue font-600 text-right">备注</td>
            <td colspan="3">
              <a-textarea v-model:value="cpEvent.gcfxcz.bz" placeholder="请输入备注" :disabled="cpEvent.state > 9" />
            </td>
          </tr>
      ...
      <div v-if="cpEvent.category == '3'">
        ...
        <table class="my-table">
          ...
          <tr>
            ...
            <td>
              <a-input v-model:value="cpEvent.cpGcpJdjc.jgmc" :disabled="cpEvent.state > 9"></a-input>
            </td>
          </tr>
          <tr>
            ...
            <td>
              <a-radio-group
                ...
                v-model:value="cpEvent.cpGcpJdjc.jcfl" :disabled="cpEvent.state > 9" />
            </td>
          </tr>
          <tr>
            <td class="text-blue font-600 text-right">主要内容</td>
            <td colspan="3">
              <a-textarea v-model:value="cpEvent.cpGcpJdjc.zynr" :disabled="cpEvent.state > 9"></a-textarea>
            </td>
          </tr>
          <tr>
            <td class="text-blue font-600 text-right">备案时间</td>
            <td>
              <a-DatePicker v-model:value="cpEvent.cpGcpJdjc.basj" :disabled="cpEvent.state > 9"></a-DatePicker>
            </td>
            <td class="text-blue font-600 text-right">检查完成时间</td>
            <td>
              <a-DatePicker v-model:value="cpEvent.cpGcpJdjc.jcwcsj" :disabled="cpEvent.state > 9"></a-DatePicker>
            </td>
          </tr>
          <tr>
            <td class="text-blue font-600 text-right">备注</td>
            <td colspan="3">
              <a-textarea v-model:value="cpEvent.cpGcpJdjc.bz" :disabled="cpEvent.state > 9"></a-textarea>
            </td>
          </tr>
          <tr>
            <td class="text-blue font-600 text-right">附件</td>
            <td colspan="2">
              <MUpload :bisId="cpEvent.id || ''" source="协同处置-gcp监督检查" :maxSize="1024 * 1024 * 50"
                :disabled="cpEvent.state > 9">
              </MUpload>
            </td>
          </tr>
      ...
      <div v-if="cpEvent.category == '4'">
        ...
        <table class="my-table">
          ...
          <tr>
            <td class="text-blue font-600 text-right">药品通用名称</td>
            <td>
              <a-input v-model:value="cpEvent.cpGtjl.yptymc" :disabled="cpEvent.state > 9"></a-input>
            </td>
            <td class="text-blue font-600 text-right">药品批准文号</td>
            <td>
              <a-input v-model:value="cpEvent.cpGtjl.yppzwh" :disabled="cpEvent.state > 9"></a-input>
            </td>
          </tr>
          <tr>
          ...
          <tr>
            ...
            <td colspan="3">
              <a-input v-model:value="cpEvent.cpGtjl.ypssxkcyr" :disabled="cpEvent.state > 9"></a-input>
            </td>
          </tr>
          <tr>
            <td class="text-blue font-600 text-right">主要内容</td>
            <td>
              <a-textarea v-model:value="cpEvent.cpGtjl.zynr" :disabled="cpEvent.state > 9"></a-textarea>
            </td>
          </tr>
          <tr>
            <td class="text-blue font-600 text-right">备注</td>
            <td colspan="3">
              <a-textarea v-model:value="cpEvent.cpGtjl.bz" :disabled="cpEvent.state > 9"></a-textarea>
            </td>
          </tr>
          <tr>
            <td class="text-blue font-600 text-right">附件</td>
            <td colspan="2">
              <MUpload :bisId="cpEvent.id || ''" source="协同处置-沟通交流" :maxSize="1024 * 1024 * 50"
                :disabled="cpEvent.state > 9">
              </MUpload>
            </td>
          </tr>
      ...
      <div class="ctl-panel" v-if="cpEvent.state < 10 || !cpEvent.state">
        <div v-if="cpEvent.category < 3">
          <table style="width: 100%" class="table table-striped table-bordered center mar_t15">
            <tr>
              <td class="text-blue font-600 text-right" style="width: 10%;">接收处室</td>
              <td class="left" style="width: 40%;">
                <JSelectDept v-model:value="params.chushi" :multiple="false"></JSelectDept>
              </td>
              <td class="text-blue font-600 text-right" style="width: 10%;">接收企业</td>
              <td class="left" style="width: 40%;">
                <a-input placeholder="请选择" v-model:value="qy.qymc" style="width: 80%;" />
                <a-button type="primary" @click="selectQy()" style="width: 15%;margin-left: 5px;"> 选择</a-button>
              </td>
            </tr>
          </table>
        </div>
        ...
        <a-row style="margin:0 auto;padding: 5px;">
          <a-col :span="24" style="text-align:center">
            <a-button type="primary" class="mr-8" @click="zancun">暂存</a-button>
            <a-popconfirm title="确认发送?" ok-text="确定" cancel-text="取消" @confirm="send">
              <a-button type="primary" class="mr-8">发送</a-button>
            </a-popconfirm>
            <!-- <a-button type="primary" class="mr-8" @click="onExportWord">导出</a-button> -->
          </a-col>
        </a-row>
      </div>
    </template>
  ...
  <a-modal v-model:visible="fkxxModel" title="反馈信息" @ok="fkxxModelOk()" width="860px">
    <div style="padding-top: 40px;">
      <a-form layout="inline">
        <a-form-item label="名称" :label-col="{ span: 6 }" :wrapper-col="{ span: 18 }">
          <a-input v-model:value="eventItem.name" style="width: 100%;" />
        </a-form-item>
        ...
        <a-form-item label="是否必填" :label-col="{ span: 10 }" :wrapper-col="{ span: 16 }">
        ...
        <a-row>
          <a-col>
            <a-form-item label="企业" :label-col="{ span: 6 }" :wrapper-col="{ span: 15 }">
              <a-input placeholder="请选择" v-model:value="qy.qymc" style="width: 400px;" />
...
export default defineComponent({
  ...
  setup() {
    ...
    async function zancun() {
      ...
      closeCurrent();
      //router.go(-1);
      go({ path: '/src/views/ypsczhjg/xtcz/rwfb/CpEventList' ,query: {  isRefresh: "1" }})
    }
