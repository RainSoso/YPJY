<template>
  <div>
    <BasicModal v-bind="$attrs" @register="register" :title="modalTitle" width="80%" wrapClassName="j-user-select-modal"
      @ok="handleOk" destroyOnClose @visible-change="visibleChange">
      <a-row>
        <a-col :span="showSelected ? 18 : 24">
          <BasicTable :columns="columns" :useSearchForm="true" :formConfig="formConfig" :api="qylist"
            :rowSelection="rowSelection" :indexColumnProps="indexColumnProps" :beforeFetch="getypmc"
...
import { qylist } from '../CpEvent.api';
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const formConfig = {
      ...
      schemas: [{
        label: '公司名称', field: 'qymc', component: 'Input', labelWidth: 110, colProps: { lg: 8, span: 2 },
      }, {
        label: '统一社会信用代码', field: 'shxydm', component: 'Input', labelWidth: 140, colProps: { lg: 8, span: 2 },
      }]
    };
    //定义表格列
    const columns = [{
      title: '社会信用代码', dataIndex: 'shxydm', sorter: true
    }, {
      title: '企业名称', dataIndex: 'qymc', sorter: true
    }, {
      title: '注册地址', sorter: true, dataIndex: 'zcdz', defaultHidden: true
    }, {
      title: '法定代表人', dataIndex: 'fddbr', defaultHidden: true
    },];
    //已选择的table信息
    const selectedTable = {
      pagination: false,
