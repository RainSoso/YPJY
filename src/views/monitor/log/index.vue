  ...
  <BasicTable @register="registerTable" :searchInfo="searchInfo" :columns="logColumns">
    <template #tableTitle>
      <a-tabs defaultActiveKey="1" @change="tabChange" size="small">
        <a-tab-pane tab="用户日志" key="1"></a-tab-pane>
        <a-tab-pane tab="流程日志" key="2"></a-tab-pane>
    ...
    <template #expandedRowRender="{ record }">
      ...
      <div v-if="searchInfo.logType == 3">
        <div style="margin-bottom: 5px">
          <a-badge status="success" style="vertical-align: middle" />
          <span style="vertical-align: middle">请求URL: {{ record.requestUrl }}</span>
        </div>
        <div style="margin-bottom: 5px">
        ...
        <div>
          <a-badge status="processing" style="vertical-align: middle" />
          <span style="vertical-align: middle">响应内容: {{ record.respContent }}</span>
        </div>
      </div>
    </template>
  </BasicTable>
