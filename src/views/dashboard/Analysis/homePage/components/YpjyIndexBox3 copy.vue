...
props.viewData.mapCurrentOption = Object.keys(mapSelectMap)[0]
const getBarItemStyle = (item) => {
  // 找到最大值
  let max = 0
  props.viewData.mapInfo.forEach(el => {
    const current = el[mapSelectMap[props.viewData.mapCurrentOption]];
  ...
  const percent
    = max === 0 ? 0 :
      (item[mapSelectMap[props.viewData.mapCurrentOption]] * 100 / max).toFixed(2)
  return {
