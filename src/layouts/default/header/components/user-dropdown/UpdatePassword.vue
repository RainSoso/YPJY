...
<script lang="ts" setup>
  ...
  async function handleSubmit() {
    try {
      ...
      defHttp.put({ url: 'sys/user/updatePassword', params }, { isTransformResponse: false }).then(async (res) => {
        if (res.success) {
  ...
  async function show(name) {
    if (!name) {
      ...
      await resetFields();
      await setFieldsValue({jobContent:userStore.getUserInfo?.jobContent})
      await clearValidate();
    }
  }
