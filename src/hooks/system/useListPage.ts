...
export function useListPage(options: ListPageOptions) {
  ...
  async function onExportXls() {
    ...
    let { url, name, title, useNameForTitle, params } = options?.exportConfig ?? {};
    let realUrl = typeof url === 'function' ? url() : url;
    if (realUrl) {
      let filename = typeof name === 'function' ? name() : name;
      //update-begin-author:taoyan date:20220507 for: erp代码生成 子表 导出报错，原因未知-
      let paramsForm = { _title_: useNameForTitle !== false ? filename : (typeof title === 'function' ? title() : title) };
      try {
        Object.assign(paramsForm, await getForm().validate())
      } catch (e) {
        console.error(e);
      }
      ...
      if (isFunction(params)) {
        paramsForm = params(paramsForm) || paramsForm;
      } else if (params) {
      ...
      } else if (options.tableProps.beforeFetch && isFunction(options.tableProps.beforeFetch)) {
        let res = (await options.tableProps.beforeFetch(paramsForm, true));
        if (res) paramsForm = res;
      }
      //update-end-author:taoyan date:20220507 for: erp代码生成 子表 导出动态设置mainId
      if (selectedRowKeys.value && selectedRowKeys.value.length > 0) {
        paramsForm['selections'] = selectedRowKeys.value.join(',');
      }
      return handleExportXls(filename as string, realUrl, filterObj(paramsForm));
...
export function useListTable(tableProps: TableProps): [
  ...
  if (typeof tableProps.beforeFetch === 'function') {
    defaultTableProps.beforeFetch = function (params) {
      ...
      // @ts-ignore
      params = tableProps.beforeFetch(params) || params;
      return params;
    };
  }
