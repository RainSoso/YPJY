<template>
  <div class="area-select">
    <!--省份-->
    <a-select v-model:value="province" @change="proChange" allowClear :disabled="disabled" v-if="!hideProvince">
      <template v-for="item in provinceOptions" :key="`${item.value}`">
        <a-select-option :value="item.value">{{ item.label }}</a-select-option>
      </template>
    </a-select>
    <!--城市-->
    <a-select v-if="level >= 2" v-model:value="city" @change="cityChange" :disabled="disabled" :allowClear="hideProvince" :style="hideProvince&&level==2?{width:'100%'}:{}"
        :placeholder="cityPlaceholder">
      <a-select-option v-if="cityChooseOption" :value="null">{{cityChooseOption}}</a-select-option>
...
<script lang="ts">
  ...
  export default defineComponent({
    ...
    props: {
      ...
      cityChooseOption: propTypes.string.def(''),
      cityPlaceholder: propTypes.string.def('请选择城市'),
    },
