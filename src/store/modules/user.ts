...
import { ROLES_KEY, TOKEN_KEY, USER_INFO_KEY, LOGIN_INFO_KEY, DB_DICT_DATA_KEY, DB_CONFIG_DATA_KEY, TENANT_ID } from '/@/enums/cacheEnum';
import { getAuthCache, setAuthCache, removeAuthCache } from '/@/utils/auth';
import { GetUserInfoModel, LoginParams, ThirdLoginParams } from '/@/api/sys/model/userModel';
import { doLogout, getUserInfo, loginApi, phoneLoginApi, thirdLogin, oauth2LoginApi,oauth2LoginApi3 } from '/@/api/sys/user';
...
export const useUserStore = defineStore({
  ...
  getters: {
    ...
    getAllConfigs(): {} {
      return this.configs || getAuthCache(DB_CONFIG_DATA_KEY);
    },
  ...
  actions: {
    ...
    setAllConfigs(configs) {
      ...
      setAuthCache(DB_CONFIG_DATA_KEY, configs);
    },
    setTenant(id){
      this.tenantid = id;
    ...
    async getUserInfoAction(): Promise<UserInfo | null> {
      ...
      const { userInfo, sysAllDictItems, sysAllConfig, roles } = await getUserInfo();
      if (userInfo) {
      ...
      if (sysAllConfig) {
        this.setAllConfigs(sysAllConfig);
      }
      return userInfo;
    },
    /**
    ...
    async oauth3Login(
      ...
      try {
        const { goHome = true, mode, ...loginParams } = params;
        const data = await oauth2LoginApi3(loginParams, mode);
        const { token } = data;
        // save token
        this.setToken(token);
        return this.afterLoginAction(goHome, data);
